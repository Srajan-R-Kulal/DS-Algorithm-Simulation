function varargout = linkedqueue(varargin)
% LINKEDQUEUE MATLAB code for linkedqueue.fig
%      LINKEDQUEUE, by itself, creates a new LINKEDQUEUE or raises the existing
%      singleton*.
%
%      H = LINKEDQUEUE returns the handle to a new LINKEDQUEUE or the handle to
%      the existing singleton*.
%
%      LINKEDQUEUE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LINKEDQUEUE.M with the given input arguments.
%
%      LINKEDQUEUE('Property','Value',...) creates a new LINKEDQUEUE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before linkedqueue_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to linkedqueue_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help linkedqueue

% Last Modified by GUIDE v2.5 18-Jul-2013 20:16:12

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @linkedqueue_OpeningFcn, ...
                   'gui_OutputFcn',  @linkedqueue_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before linkedqueue is made visible.
function linkedqueue_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to linkedqueue (see VARARGIN)

% Choose default command line output for linkedqueue
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes linkedqueue wait for user response (see UIRESUME)
...
top(i)=annotation('textbox',[x+0.13 y-0.2 w h]);
ta(i)=annotation('arrow',X1,Y1,'Linewidth',2);
set(top(i),'String','REAR');

X1=[x+0.1,x+0.1];
Y1=[y-0.1 y];

top1(i)=annotation('textbox',[x+0.08 y-0.2 w h]);
ta1(i)=annotation('arrow',X1,Y1,'Linewidth',2);
set(top1(i),'String','FRONT');
...
set(top1(i),'Visible','off');
   set(ta1(i),'Visible','off');
...
function varargout = linkedqueue_OutputFcn(hObject, eventdata, handles) 
...
if n==1
    set(top1(n),'Visible','on');
   set(ta1(n),'Visible','on');
end
...
if n==0
    ...
    set(top1(n),'Visible','off');
    set(ta1(n),'Visible','off');
    end
